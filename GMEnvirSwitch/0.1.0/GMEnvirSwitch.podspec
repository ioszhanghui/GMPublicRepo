#
# Be sure to run `pod lib lint GMEnvirSwitch.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see https://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'GMEnvirSwitch'
  s.version          = '0.1.0'
  s.summary          = '切换测试环境'

# This description is used to generate tags and improve search results.
#   * Think: What does it do? Why did you write it? What is the focus?
#   * Try to keep it short, snappy and to the point.
#   * Write the description between the DESC delimiters below.
#   * Finally, don't worry about the indent, CocoaPods strips it!

  s.description      = <<-DESC
国美环境切换
                       DESC

  s.homepage         = 'https://github.com/ioszhanghui/GMEnvirSwitch.git'
  # s.screenshots     = 'www.example.com/screenshots_1', 'www.example.com/screenshots_2'
  s.license          = { :type => 'MIT', :file => 'LICENSE' }
  s.author           = { 'ioszhanghui@163.com' => 'yomingyo@gmail.com' }
  s.source           = { :git => 'https://github.com/ioszhanghui/GMEnvirSwitch.git', :tag => s.version.to_s }
  # s.social_media_url = 'https://twitter.com/<TWITTER_USERNAME>'

  s.ios.deployment_target = '9.0'
  s.requires_arc = true
  s.source_files = 'GMEnvirSwitch/Classes/**/*'
  
  
  s.prefix_header_contents = '#import<GMmacros/macros.h>','#import<Masonry/Masonry.h>','#import<MJExtension/MJExtension.h>','#import<GMDataTool/GM_DataHeader.h>','#import<GMSandboxTool/GMFileUtil.h>'
  
  s.resources = ['GMEnvirSwitch/Assets/*.png']

  # s.public_header_files = 'Pod/Classes/**/*.h'
  # s.frameworks = 'UIKit', 'MapKit'
    s.dependency 'Masonry'
    s.dependency 'GMmacros'
    s.dependency 'MJExtension'
    s.dependency 'GMSandboxTool'
    s.dependency 'GMDataTool'
    
end
